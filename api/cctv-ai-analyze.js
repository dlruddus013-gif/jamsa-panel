...
export default async function handler(req, res) {
  ...
  try {
    ...
    const promptText = `당신은 한국잠사박물관의 CCTV 보안 분석 AI입니다. 이 CCTV 이미지를 분석하여 위험도를 판단하고 JSON으로만 응답하세요 (코드블록/설명 절대 금지, 순수 JSON만):
...
판단 기준 (한국잠사박물관 운영 맥락):
- DANGER (80-100): 화재/연기, 침입(영업외 시간 인적), 폭력, 누수/물난리, 사고
- WARNING (40-79): 미확인 큰 움직임, 영업시간 외 차량, 동물 진입, 비정상 혼잡
